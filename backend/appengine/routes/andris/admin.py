...
from routes.andris.categoria.salvarCategoria import salvar
...
def index():
    ...
    for cat in categorias:
        key = cat.key
        key_id = key.id()
        #cat.editar_path = router.to_path(editar_path, key_id)
        cat.deletar_path = router.to_path(deletar_path, key_id)


    contexto = {'categoria_lista':categorias,'salvar_path':salvar_path,'editar_path':"",'deletar_path':deletar_path}
